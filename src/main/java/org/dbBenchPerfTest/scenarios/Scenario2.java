...
import org.testController.PrepareScenarioEnvironment;
    ...
    public void run(DatabaseInface db) throws Exception {
        ...
        try (Statement stmt = config.getConn().createStatement()) {
            for (int i = 0; i < dates.length; i++) {
                ...
                boolean tableExists = PrepareScenarioEnvironment.checkTableIsExist(config.getConn(), targetTable, config.getDbType(), owner);
                if (tableExists) {
                    try(Statement stmtTruncate = config.getConn().createStatement()){
                        stmtTruncate.execute("DROP TABLE "+ targetTable);
                        logger.info("清空 "+ targetTable + " 表成功");
                    }
                }
